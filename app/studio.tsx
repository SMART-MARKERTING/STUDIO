...
export default function ContentStudio({ embedded = false }: { embedded?: boolean }) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.warn(
        "Content Studio local storage is full; export a backup and remove large uploaded assets.",
      );
    }
  }, [hydrated, studio]);
  ...
  function updatePost(next: StudioPost, action = "Content updated") {
    ...
    setStudio((current) => ({
      ...
      posts: current.posts.map((post) =>
        post.id === stamped.id ? stamped : post,
      ),
  ...
  function createPost() {
    ...
    setStudio((current) => ({
      ...
      audit: [addAudit("Draft created", post.title), ...current.audit],
    }));
    setSelectedId(post.id);
    notify(
      "New draft created. Add the message and review details before approval.",
      "info",
    );
  }
  ...
  function deletePost(id: string) {
    ...
    setStudio((current) => ({
      ...
      audit: [addAudit("Content deleted", target.title), ...current.audit],
    }));
    setSelectedId(null);
  ...
  function duplicatePost(post: StudioPost) {
    ...
    setStudio((current) => ({
      ...current,
      posts: [copy, ...current.posts],
      audit: [addAudit("Content duplicated", post.title), ...current.audit],
    }));
  ...
  function approvePost(post: StudioPost) {
    if (!post.title.trim() || !post.platforms.length)
      return notify(
        "Add a title and at least one platform before approval.",
        "error",
      );
    if (postHasPlaceholder(post))
      return notify(
        "Placeholder copy cannot be approved. Replace every draft instruction with the final caption.",
        "error",
      );
    if (post.platforms.some((p) => !post.captions[p].trim()))
      return notify("Every selected platform needs a caption.", "error");
    if (riskRequiresSource(post))
      return notify(
        "Yellow and red content requires both a verified source and a disclosure record.",
        "error",
      );
    updatePost(
      { ...post, status: "Approved", lastError: "" },
      "Content approved",
    );
  ...
  async function publishPost(post: StudioPost) {
    if (studio.paused)
      return notify(
        "Publishing is paused. Resume the workspace first.",
        "error",
      );
    if (post.status !== "Approved" && post.status !== "Scheduled")
      return notify("Approve this content before publishing.", "error");
    if (postHasPlaceholder(post))
      return notify(
        "Publishing blocked: this content still contains placeholder copy.",
        "error",
      );
    const asset = studio.assets.find((item) => item.id === post.assetId);
    if (asset && asset.approval !== "Approved")
      return notify(
        "This creative is not approved. Choose or approve another asset.",
        "error",
      );
    ...
    try {
      if (asset?.source === "upload")
        return notify(
          "This uploaded creative is stored only in your browser. Choose a bundled asset or no creative before sending it live.",
          "error",
        );
      const assetUrl = asset
        ? new URL(asset.src, window.location.origin).toString()
        : undefined;
      const response = await fetch("/api/publish", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ post, assetUrl }),
      });
      ...
      if (!response.ok)
        throw new Error(
          result.error || "The publishing provider rejected the request.",
        );
      ...
      updatePost(
        {
          ...post,
          status,
          providerPostIds: result.providerPostIds ?? [],
          lastError: result.warnings?.join(" ") ?? "",
        },
        status === "Scheduled" ? "Content scheduled" : "Content published",
      );
      notify(
        result.warnings?.length
          ? `${status}. Some channels need attention: ${result.warnings.join(" ")}`
          : status === "Scheduled"
            ? "Scheduled with the connected social provider."
            : "Published to the connected channels.",
  ...
  async function generateBatch() {
    ...
    try {
      const response = await fetch("/api/generate-batch", {
        ...
        headers: { "content-type": "application/json" },
        body: JSON.stringify({
          existingCount: studio.posts.length,
          assets: studio.assets.map(({ id, title, category, approval }) => ({
            id,
            title,
            category,
            approval,
      ...
      const result = (await response.json()) as {
        posts?: Array<Partial<StudioPost> & { caption?: string }>;
        error?: string;
        source?: string;
      };
      if (!response.ok || !result.posts?.length)
        throw new Error(result.error || "AI generation is not configured.");
      const batch = normalizeGeneratedPosts(
        result.posts,
        studio.assets,
        studio.posts.length,
      );
      setStudio((current) => ({
        ...current,
        posts: [...batch, ...current.posts],
        audit: [
          addAudit(
            "Weekly batch generated",
            `${batch.length} OpenAI-assisted drafts created`,
          ),
          ...current.audit,
        ],
      }));
      setActive("Approvals");
      notify(
        `${batch.length} AI-assisted drafts created and routed to review.`,
      );
    } catch {
      ...
      setStudio((current) => ({
        ...
        audit: [
          addAudit(
            "Weekly batch generated",
            `${batch.length} approved-library drafts created without external AI`,
          ),
          ...current.audit,
        ],
      }));
      setActive("Approvals");
      notify(
        "7 compliant library-based drafts created. Add OPENAI_API_KEY to enable live AI generation.",
        "info",
      );
    } finally {
      setGenerating(false);
    ...
    setStudio((current) => ({
      ...
      audit: [
        addAudit(
          ...
          "Workspace-wide control changed by administrator",
        ),
        ...current.audit,
      ],
    }));
    notify(
      paused ? "All outbound publishing is paused." : "Publishing resumed.",
      paused ? "info" : "success",
    );
  }
    ...
    setStudio((current) => ({
      ...
      audit: [
        addAudit(
          ...
          `${channels.filter((c) => c.connected).length} channels connected`,
        ),
        ...current.audit,
      ],
    }));
    ...
    <main className={`studio-shell ${embedded ? "studio-embedded" : ""}`}>
      <aside className="sidebar">
        ...
        <nav aria-label="Content Studio">
          {nav.map((item) => (
            ...
            >
              ...
              {item}
              {item === "Approvals" && counts.review > 0 ? (
                <b>{counts.review}</b>
              ) : null}
            </button>
          ))}
        ...
        <div className="sidebar-bottom">
          <div className="mini-profile">
            <img src="/assets/headshot.jpeg" alt="Mykoal DeShazo" />
            <div>
              <strong>Mykoal DeShazo</strong>
      ...
      <section className="workspace">
        ...
        <header className="topbar">
          ...
          <div className="top-actions">
            <button
              className={studio.paused ? "pause on" : "pause"}
              onClick={togglePause}
            >
              <i />
              {studio.paused ? "Publishing paused" : "Pause all"}
            </button>
            <button
              className="primary"
              ...
              disabled={generating}
            >
              {generating ? "Building batch…" : "+ Generate weekly batch"}
            </button>
          </div>
        </header>
        {active === "Assets" ? (
          <AssetsView studio={studio} setStudio={setStudio} notify={notify} />
        ) : active === "Approvals" ? (
          <ApprovalsView
            posts={studio.posts}
            open={setSelectedId}
      ...
      {selected ? (
        <PostDrawer
          ...
          publishing={publishingId === selected.id}
      ...
      {notice ? (
        <div className={`toast ${notice.tone}`}>
          {notice.tone === "error" ? "!" : notice.tone === "info" ? "i" : "✓"}{" "}
          {notice.message}
        </div>
      ) : null}
    </main>
...
function navIcon(item: ViewName) {
  return (
    {
      Overview: "⌂",
      Calendar: "□",
...
function Overview({
  ...
  const impressions = studio.posts.reduce(
  ...
  const health = Math.max(
    65,
    Math.min(99, 78 + counts.approved + counts.scheduled - counts.review * 2),
  );
  return (
    <div className="page-content">
      <section className="welcome">
        <div>
          ...
          <h2>Your content engine is ready.</h2>
          <p>
            {studio.paused
              ? "Outbound publishing is paused. Drafting, review, and approvals remain available."
              : `${counts.review} item${counts.review === 1 ? "" : "s"} need review. Approved content can be sent to connected channels when provider credentials are configured.`}
          </p>
          <div className="welcome-actions">
            <button onClick={generate}>Generate next batch</button>
            <button className="ghost" onClick={() => navigate("Calendar")}>
              View full calendar →
            </button>
          </div>
        </div>
      ...
      <div className="metrics">
        <Metric
          ...
          note={`${studio.posts.length} total content items`}
          accent="cyan"
        />
        <Metric
          label="Awaiting approval"
          value={String(counts.review)}
          note={`${counts.draft} drafts in progress`}
          accent="lime"
        />
        <Metric
          label="Recorded impressions"
          value={compact(impressions)}
          note="Synced + local metrics"
          accent="blue"
        />
        <Metric
          label="Attributed leads"
          value={String(leads)}
          note="Across active content"
          accent="gold"
        />
      </div>
          ...
          {studio.posts
            ...
            .map((post) => (
              <PostRow
                key={post.id}
                post={post}
                asset={studio.assets.find((a) => a.id === post.assetId)}
                open={open}
              />
            ))}
        </section>
          ...
          <section className="panel">
            ...
            <div className="channel-list">
              {studio.channels.map((channel) => (
                <div key={channel.id}>
                  <i style={{ background: channel.color }}>{channel.mark}</i>
                  <span>
                  ...
                  <b className={channel.connected ? "dot ok" : "dot"} />
                </div>
              ))}
            </div>
          </section>
          <section className="panel compliance">
            ...
            <div className="risk-bars">
              ...
              <div>
                ...
                <b>{studio.posts.filter((p) => p.risk === "Yellow").length}</b>
              </div>
              <div>
                <span>
                  <i className="red" />
                ...
                <b>{studio.posts.filter((p) => p.risk === "Red").length}</b>
              </div>
            </div>
            <p>
              Rate, payment, savings, qualification, and expiring claims require
              verified source data and accountable approval.
            </p>
          </section>
        </aside>
      </div>
...
function PostRow({
  ...
  return (
    <button className="post-row" onClick={() => open(post.id)}>
      <div className="date-tile">
        <span>{schedule.day}</span>
        <b>{schedule.date}</b>
      </div>
      <img src={asset?.src || "/assets/smart-marketing-logo.jpg"} alt="" />
      <div className="post-copy">
        <strong>{post.title}</strong>
        <span>
          {schedule.time} · {post.pillar}
        </span>
        {post.lastError ? <em>{post.lastError}</em> : null}
      </div>
      <div className="platforms">
        {post.platforms.map((p) => (
          <i key={p} title={platformLabels[p]}>
            {platformMarks[p]}
          </i>
        ))}
      </div>
      <span className={`badge ${post.risk.toLowerCase()}`}>{post.risk}</span>
      <span
        className={`status ${post.status.replaceAll(" ", "-").toLowerCase()}`}
      >
        {post.status}
      </span>
      <b className="arrow">›</b>
  ...
  return (
    <div className="page-content">
      <section className="section-heading">
        <div>
          <span>
            {title === "Calendar" ? "PUBLISHING PLAN" : "CONTENT LIBRARY"}
          </span>
          <h2>{title === "Content" ? "All content" : "Content calendar"}</h2>
          <p>Search, filter, edit, and schedule each platform-ready version.</p>
        </div>
      ...
      <div className="filter-bar">
        ...
        <select
          ...
          value={status}
          onChange={(e) => setStatus(e.target.value as "all" | PostStatus)}
        >
...
function ApprovalsView({
  ...
  posts: StudioPost[];
  open: (id: string) => void;
  approve: (post: StudioPost) => void;
  notify: (m: string, t?: Notice["tone"]) => void;
}) {
  const review = posts.filter((p) => p.status === "Needs review");
  function bulkApprove() {
    const green = review.filter(
      (p) =>
        p.risk === "Green" &&
        p.title.trim() &&
        p.platforms.every((platform) => p.captions[platform].trim()),
    );
    if (!green.length)
      return notify(
        "No eligible Green items are waiting for approval.",
        "info",
      );
    green.forEach(approve);
    notify(
      `${green.length} Green item${green.length === 1 ? "" : "s"} approved. Yellow and red items still require individual review.`,
    );
  }
  return (
    <div className="page-content">
      <section className="section-heading">
        <div>
          ...
          <p>
            Open each item to review platform copy, sources, disclosures,
            creative, and timing.
          </p>
        </div>
        <button onClick={bulkApprove}>Approve eligible Green</button>
      </section>
      <section className="panel approval-list">
        {review.length ? (
          review.map((post) => (
            <article className="approval-card" key={post.id}>
              <div>
                <span className={`badge ${post.risk.toLowerCase()}`}>
                  {post.risk}
                </span>
                <h3>{post.title}</h3>
                <p>
                  {post.pillar} ·{" "}
                  {post.platforms.map((p) => platformLabels[p]).join(", ")}
                </p>
              </div>
              <div className="approval-actions">
                <button onClick={() => open(post.id)}>Review details</button>
                {post.risk === "Green" ? (
                  <button className="primary" onClick={() => approve(post)}>
                    Approve
                  </button>
                ) : null}
              </div>
          ...
          <div className="empty">
            Everything in the review queue has a decision.
          </div>
        )}
      </section>
    </div>
...
function AssetsView({
  ...
  async function upload(event: ChangeEvent<HTMLInputElement>) {
    ...
    if (!file.type.startsWith("image/"))
      return notify("Choose a PNG, JPG, or WebP image.", "error");
    try {
      ...
      setStudio((current) => ({
        ...
        audit: [addAudit("Asset uploaded", asset.title), ...current.audit],
      }));
  ...
  function change(asset: StudioAsset, values: Partial<StudioAsset>) {
    setStudio((current) => ({
      ...current,
      assets: current.assets.map((a) =>
        a.id === asset.id ? { ...a, ...values } : a,
      ),
      audit: values.approval
        ? [
            addAudit(
              "Asset approval changed",
              `${asset.title}: ${values.approval}`,
            ),
            ...current.audit,
          ]
        : current.audit,
  ...
  function remove(asset: StudioAsset) {
    if (asset.source === "bundled")
      return notify(
        "Bundled compliance assets are locked. Change their approval state instead.",
        "info",
      );
    if (!window.confirm(`Delete “${asset.title}”?`)) return;
    setStudio((current) => ({
      ...current,
      assets: current.assets.filter((a) => a.id !== asset.id),
      posts: current.posts.map((p) =>
        p.assetId === asset.id ? { ...p, assetId: "" } : p,
      ),
      audit: [addAudit("Asset deleted", asset.title), ...current.audit],
    }));
    notify("Asset deleted.", "info");
  }
  function download(asset: StudioAsset) {
    const a = document.createElement("a");
    a.href = asset.src;
    a.download = `${asset.title.replace(/[^a-z0-9]+/gi, "-").toLowerCase()}.jpg`;
    a.click();
  }
  return (
    <div className="page-content">
      <section className="section-heading">
        <div>
          ...
          <h2>Brand assets</h2>
          <p>
            Uploaded files remain in this browser until a shared media provider
            is configured.
          </p>
        </div>
          ...
          <article className="asset-card" key={asset.id}>
            ...
            <div>
              <input
                ...
                onChange={(e) => change(asset, { title: e.target.value })}
              />
              <div className="asset-controls">
                <select
                  ...
                  onChange={(e) =>
                    change(asset, {
                      category: e.target.value as StudioAsset["category"],
                    })
                  }
                >
                  {["HELOC", "DSCR", "Brand", "Headshot", "Other"].map((c) => (
                    <option key={c}>{c}</option>
                  ))}
                </select>
                <select
                  aria-label="Asset approval"
                  value={asset.approval}
                  onChange={(e) =>
                    change(asset, {
                      approval: e.target.value as StudioAsset["approval"],
                    })
                  }
                >
              ...
              <small>
                {asset.source === "bundled"
                  ? "Locked campaign asset"
                  : "Saved in this browser"}
              </small>
              <div className="card-actions">
                <button onClick={() => download(asset)}>Download</button>
                <button onClick={() => remove(asset)}>Delete</button>
              </div>
            </div>
          </article>
        ))}
...
function AnalyticsView({
  ...
  const filtered = posts.filter(
    (p) => platform === "all" || p.platforms.includes(platform),
  );
  const totals = filtered.reduce(
    (a, p) => ({
      impressions: a.impressions + p.metrics.impressions,
      clicks: a.clicks + p.metrics.clicks,
      leads: a.leads + p.metrics.leads,
      applications: a.applications + p.metrics.applications,
    }),
    { impressions: 0, clicks: 0, leads: 0, applications: 0 },
  );
  const top = [...filtered]
    .sort(
      (a, b) =>
        b.metrics.leads - a.metrics.leads ||
  ...
  function exportCsv() {
    ...
    a.download = "content-studio-analytics.csv";
    a.click();
  ...
  async function sync() {
    ...
    try {
      const response = await fetch(
        `/api/analytics${platform === "all" ? "" : `?platform=${platform}`}`,
      );
      const result = (await response.json()) as {
        error?: string;
        message?: string;
      ...
      if (!response.ok)
        throw new Error(result.error || "Analytics sync failed.");
      ...
      const updates = new Map<string, StudioPost["metrics"]>();
      for (const post of posts) {
        const related = records.filter((record) =>
          post.providerPostIds.includes(providerRecordId(record)),
        );
        if (!related.length) continue;
        updates.set(
          post.id,
          related.reduce<StudioPost["metrics"]>(
            (total, record) => {
              const metrics = providerMetrics(record);
              return {
                impressions: total.impressions + metrics.impressions,
                clicks: total.clicks + metrics.clicks,
                leads: total.leads + metrics.leads,
                applications: total.applications + metrics.applications,
              };
            },
            { impressions: 0, clicks: 0, leads: 0, applications: 0 },
          ),
      ...
      setStudio((current) => ({
        ...
        audit: [
          addAudit(
            ...
            `${records.length} provider records retrieved`,
          ),
          ...current.audit,
        ],
      }));
      ...
      notify(
        matched
          ? `${result.message || "Provider analytics received"} ${matched} local content item${matched === 1 ? "" : "s"} updated.`
          : `${result.message || "Provider analytics received"} No returned post IDs matched this browser's published content.`,
    ...
    <div className="page-content">
      <section className="section-heading">
        <div>
          ...
          <p>
            Local content IDs, campaign IDs, platform versions, and recorded
            outcomes.
          </p>
        </div>
      ...
      <div className="metrics">
        <Metric
          ...
          note={`${filtered.length} content items`}
          accent="cyan"
        />
        <Metric
          label="Link clicks"
          value={compact(totals.clicks)}
          note={`${totals.impressions ? ((totals.clicks / totals.impressions) * 100).toFixed(1) : "0.0"}% recorded CTR`}
          accent="lime"
        />
        <Metric
          label="Leads"
          value={String(totals.leads)}
          note={`${totals.clicks ? ((totals.leads / totals.clicks) * 100).toFixed(1) : "0.0"}% click-to-lead`}
          accent="blue"
        />
        <Metric
          label="Applications"
          value={String(totals.applications)}
          note={`${totals.leads ? Math.round((totals.applications / totals.leads) * 100) : 0}% of leads`}
          accent="gold"
        />
      </div>
          ...
          {top.length ? (
            top.map((p) => (
              <i
                key={p.id}
                style={{
                  height: `${Math.max(6, Math.round((p.metrics.clicks / max) * 150))}px`,
                }}
                title={`${p.title}: ${p.metrics.clicks} clicks`}
              >
                <span>{p.title.slice(0, 8)}</span>
              </i>
            ))
          ) : (
...
function SettingsView({
  ...
  async function refresh() {
    ...
    try {
      ...
      const result = (await response.json()) as {
        ...
        paidPlan?: boolean;
        channels?: Partial<
          Record<Platform, { connected: boolean; handle?: string }>
        >;
        error?: string;
      };
      if (!response.ok)
        throw new Error(result.error || "Unable to check integrations.");
      const now = new Date().toISOString();
      updateChannels(
        studio.channels.map((channel) => ({
          ...channel,
          connected: Boolean(result.channels?.[channel.id]?.connected),
          handle: result.channels?.[channel.id]?.handle || "",
          lastChecked: now,
        })),
      );
      setProvider({
        configured: Boolean(result.configured),
        ai: Boolean(result.modelConfigured),
        restricted: Boolean(result.adminRestricted),
        paid: Boolean(result.paidPlan),
      });
      notify(
        result.configured
          ? "Connection status refreshed from Ayrshare."
          : "Ayrshare is not configured yet; no channels were marked connected.",
  ...
  function openProvider() {
    ...
    notify("Ayrshare social account setup opened in a new tab.", "info");
  }
  function exportWorkspace() {
    const a = document.createElement("a");
    a.href = URL.createObjectURL(
      new Blob([JSON.stringify(studio, null, 2)], { type: "application/json" }),
    );
    a.download = `content-studio-backup-${new Date().toISOString().slice(0, 10)}.json`;
    a.click();
    URL.revokeObjectURL(a.href);
    notify("Workspace backup exported.");
  }
  function reset() {
    if (
      !window.confirm(
        "Reset posts, assets, approvals, and local connection status to the starter workspace?",
      )
    )
      return;
  ...
  return (
    <div className="page-content">
      <section className="section-heading">
        <div>
          <span>WORKSPACE</span>
          <h2>Connections & controls</h2>
          <p>
            Secrets remain server-side. Connection badges only turn green after
      ...
      <section className="panel settings-list">
        {studio.channels.map((c) => (
          <div key={c.id}>
            <i style={{ background: c.color }}>{c.mark}</i>
            <span>
              <strong>{c.name}</strong>
              <small>
                {c.connected
                  ? `${c.handle || "Professional account"} · verified ${c.lastChecked ? new Date(c.lastChecked).toLocaleString() : "now"}`
                  : "Not connected"}
              </small>
            </span>
            <b
              className={
                c.connected ? "connection-state connected" : "connection-state"
              }
            >
              {c.connected ? "Connected" : "Setup needed"}
            </b>
      ...
      <div className="settings-grid">
        <section className="panel settings-card">
          ...
          <p>
            ...
            {provider.configured
              ? "Configured"
              : "Check status or add AYRSHARE_API_KEY"}
          </p>
          <p>
            <b>Provider branding:</b>{" "}
            {provider.paid
              ? "Paid plan confirmed — no free-plan label"
              : "Blocked until a paid Ayrshare plan is confirmed"}
          </p>
          <p>
            <b>AI generation:</b>{" "}
            {provider.ai
              ? "Configured"
              : "Uses compliant local fallback until OPENAI_API_KEY is added"}
          </p>
          <p>
            <b>Admin restriction:</b>{" "}
            {provider.restricted
              ? "Enabled"
              : "Optional STUDIO_ADMIN_EMAIL is not set"}
          </p>
          <button onClick={refresh}>Test services</button>
        </section>
        <section className="panel settings-card">
          <span>PORTABILITY</span>
          <h3>Workspace data</h3>
          <p>
            Posts, approvals, uploads, and audit events persist in this browser.
      ...
      <section className="panel audit-log">
        ...
        {studio.audit.slice(0, 12).map((event) => (
          <div key={event.id}>
...
function PostDrawer({
  ...
  publishing: boolean;
  close: () => void;
  save: (p: StudioPost, a?: string) => void;
  approve: (p: StudioPost) => void;
  publish: (p: StudioPost) => void;
  duplicate: (p: StudioPost) => void;
  remove: (id: string) => void;
}) {
  const [draft, setDraft] = useState(post);
  const selectedAsset = assets.find((a) => a.id === draft.assetId);
  function change<K extends keyof StudioPost>(key: K, value: StudioPost[K]) {
  ...
  function approveDraft() {
    ...
    if (
      ...
      draft.platforms.every((platform) => draft.captions[platform].trim()) &&
      !riskRequiresSource(draft) &&
      !postHasPlaceholder(draft)
    )
      setDraft((current) => ({
        ...current,
        status: "Approved",
  ...
  return (
    <div className="drawer-backdrop" onMouseDown={close}>
      <aside className="drawer" onMouseDown={(e) => e.stopPropagation()}>
        ...
        <img
          className="drawer-art"
          src={selectedAsset?.src || "/assets/smart-marketing-logo.jpg"}
          alt="Selected creative"
        />
        <div className="drawer-meta">
          <span className={`badge ${draft.risk.toLowerCase()}`}>
            {draft.risk} compliance
          </span>
          <span>
            {draft.status} · {formatSchedule(draft.scheduledAt).time}
          </span>
        </div>
        <section className="form-section">
          ...
          <div className="form-grid">
            ...
            <label>
              ...
              <select
                value={draft.risk}
                onChange={(e) =>
                  change("risk", e.target.value as StudioPost["risk"])
                }
              >
                <option>Green</option>
            ...
            <label>
              ...
              <input
                ...
                onChange={(e) =>
                  change("scheduledAt", new Date(e.target.value).toISOString())
                }
              />
            </label>
            <label>
          ...
          <div className="platform-toggles">
            {platforms.map((platform) => (
              <label
                key={platform}
                className={draft.platforms.includes(platform) ? "selected" : ""}
              >
                <input
                  type="checkbox"
                ...
                />
                {platformMarks[platform]} {platformLabels[platform]}
              </label>
            ))}
          </div>
        </section>
        <section className="form-section">
          ...
          <label>
            Always add hashtags
            <input
              placeholder="#MortgageEducation #ArizonaHomes"
              value={draft.hashtags}
              onChange={(e) => change("hashtags", e.target.value)}
            />
            <small>
              Automatically added to every selected platform without
              duplicating tags already in a caption.
            </small>
          </label>
          <label>
            Post location
          ...
          {draft.platforms.map((platform) => (
            <label key={platform}>
              {platformLabels[platform]} caption
              <textarea
                rows={4}
                value={draft.captions[platform]}
                onChange={(e) =>
                  change("captions", {
                    ...draft.captions,
        ...
        <section className="form-section">
          ...
          {riskRequiresSource(draft) ? (
            <p className="validation">
              Source and disclosure are required before this {draft.risk} item
              can be approved.
            </p>
          ) : null}
          <label>
            Verified source
            <input
              placeholder="Article, guideline, or campaign source"
              value={draft.source}
          ...
          {draft.lastError ? (
            <p className="validation">
              <b>Last publishing attempt:</b> {draft.lastError}
            </p>
          ) : null}
        </section>
        <footer className="drawer-footer">
          <div>
            <button className="danger-link" onClick={() => remove(draft.id)}>
              Delete
            </button>
            <button onClick={() => duplicate(draft)}>Duplicate</button>
          </div>
          <div>
            <button
              onClick={() => {
                change("status", "Draft");
                save({ ...draft, status: "Draft" }, "Returned to draft");
              }}
            >
              Return to draft
            </button>
            <button onClick={store}>Save</button>
            <button className="approve-button" onClick={approveDraft}>
              Approve
            </button>
            <button
              className="primary"
              disabled={
                publishing ||
                paused ||
                !(["Approved", "Scheduled"] as PostStatus[]).includes(
                  draft.status,
                )
              }
...
function providerRecordId(record: Record<string, unknown>) {
  return String(record.id || record.postId || record.refId || "");
}
function providerMetrics(record: Record<string, unknown>) {
  const nested = (
  ...
  return {
    ...
    leads: number(record.leads, nested.leads),
    applications: number(record.applications, nested.applications),
  };
}

function compressImage(file: File): Promise<string> {
  if (file.size > 12 * 1024 * 1024)
    return Promise.reject(new Error("Images must be smaller than 12 MB."));
