...
export default function ContentStudio() {
  ...
  function approvePost(post: StudioPost) {
    ...
    if (postHasPlaceholder(post)) return notify("Placeholder copy cannot be approved. Replace every draft instruction with the final caption.", "error");
  ...
  async function publishPost(post: StudioPost) {
    ...
    if (postHasPlaceholder(post)) return notify("Publishing blocked: this content still contains placeholder copy.", "error");
...
function PostDrawer({ post, assets, paused, publishing, close, save, approve, publish, duplicate, remove }: { post:StudioPost;assets:StudioAsset[];paused:boolean;publishing:boolean;close:()=>void;save:(p:StudioPost,a?:string)=>void;approve:(p:StudioPost)=>void;publish:(p:StudioPost)=>void;duplicate:(p:StudioPost)=>void;remove:(id:string)=>void }) {
  ...
  function approveDraft(){approve(draft);if(draft.title.trim()&&draft.platforms.length&&draft.platforms.every((platform)=>draft.captions[platform].trim())&&!riskRequiresSource(draft)&&!postHasPlaceholder(draft))setDraft((current)=>({...current,status:"Approved",lastError:""}));}
